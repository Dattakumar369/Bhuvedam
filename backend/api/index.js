...
async function getFarmerByPhone(phone) {
  return db.query.farmers.findFirst({
    where: (0, import_drizzle_orm21.eq)(farmers.phone, formatPhone(phone))
  });
}
...
async function findFarmerByIdentifier(identifier) {
  ...
  if (trimmed.includes("@")) {
    ...
    return db.query.farmers.findFirst({ where: (0, import_drizzle_orm22.eq)(farmers.email, email) });
  }
  const phone = formatPhone(trimmed);
  return db.query.farmers.findFirst({ where: (0, import_drizzle_orm22.eq)(farmers.phone, phone) });
}
async function findFarmerByPhoneOrEmail(phone, email) {
  ...
  return db.query.farmers.findFirst({
    where: conditions.length === 1 ? conditions[0] : (0, import_drizzle_orm22.or)(...conditions)
  });
}
...
async function changeFarmerPassword(farmerId, currentPassword, newPassword) {
  const farmer = await db.query.farmers.findFirst({ where: (0, import_drizzle_orm22.eq)(farmers.id, farmerId) });
